...
# Wait for a real handshake, not just an open port: Docker binds the published
# port as soon as the container exists, so a TCP probe passes while the process
# inside is still starting -- and the suite then runs against nothing and writes
# an empty report.
python3 "$HERE/wait-origin.py" "$ORIGIN_PORT" || die "the echo origin never became ready"
